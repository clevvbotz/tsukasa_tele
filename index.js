...
module.exports = alpha = async (alpha, bot) => {
	...
	try {
		...
		//get type message 
		var typeMessage = body.substr(0, 50)
			.replace(/\n/g, '')
		...
		case "help": {
			...
			alpha.replyWithVoice({
				source: fs.createReadStream(`./audio/${audio}.mp3`)
			})
		}
		break
		//anime
...
 Description : ${res[v].description}\n\n---------------------------\n\n`
		...
		case "instareel": {
			...
			var result = await instagramdl(args[0])
			...
			for (let {
				...
				of result) alpha.replyWithVideo({
				url: url
			}, {
				caption: lang.ok
			})
		...
		case "fbdl": {
			...
			var {
				result
			} = await facebookdl(args[0])
				.catch(async _ => await facebookdlv2(args[0]))
				.catch(async _ => await facebookdlv3(args[0]))
				.catch(async _ => await aiovideodl(args[0]))
			for (let {
					url
				}
				of result.reverse()) alpha.replyWithVideo({
				url: url
			}, {
				caption: lang.ok
			})
		...
		case "twtdl": {
			...
			for (let x = 0; x < res.media.length; x++) {
				var capt = x === 0 ? res.caption.replace(/https:\/\/t.co\/[a-zA-Z0-9]+/gi, '')
					.trim() : ''
				alpha.replyWithVideo({
					url: res.media[x].url
				}, {
		...
		case 'kbbi': {
			...
			var {
				data
			} = await fetch(`https://api.lolhuman.xyz/api/kbbi?apikey=maslent&query=${text}`)
		...
		case 'brainly': {
			...
			var {
				data
			} = await fetch(`https://api.lolhuman.xyz/api/brainly?apikey=maslent&query=${text}`)
		...
		case 'roboguru': {
			...
			var {
				data
			} = await axios.get(`https://api.lolhuman.xyz/api/roboguru?apikey=maslent&query=${text}&grade=sma&subject=sejarah`)
		...
		case 'jarak': {
			...
			var {
				data
			} = await axios.get(`https://api.lolhuman.xyz/api/jaraktempuh?apikey=maslent&kota1=${titt1}&kota2=${titt2}`)
		...
		case 'translate': {
			...
			var {
				data
			} = await fetch(`https://api.lolhuman.xyz/api/translate/auto/${kode_negara}?apikey=maslent&text=${tittt}`)
		...
		case 'jadwaltv': {
			...
			var {
				data
			} = await fetch(`https://api.lolhuman.xyz/api/jadwaltv/${text}?apikey=maslent`)
			reply(lang.wait)
		...
		case 'jadwaltvnow': {
			var {
				data
			} = await fetch(`https://api.lolhuman.xyz/api/jadwaltv/now?apikey=maslent`)
			reply(lang.wait)
		...
		case 'newsinfo': {
			var {
				data
			} = await fetch(`https://api.lolhuman.xyz/api/newsinfo?apikey=maslent`)
			reply(lang.wait)
			var titttttt = `Result :\n${data.result}`
		...
		case 'cnnindonesia': {
			var {
				data
			} = await fetch(`https://api.lolhuman.xyz/api/cnnindonesia?apikey=maslent`)
			reply(lang.wait)
		...
		case 'cnnnasional': {
			var {
				data
			} = await fetch(`https://api.lolhuman.xyz/api/cnnindonesia/nasional?apikey=maslent`)
			reply(lang.wait)
		...
		case 'cnninternasional': {
			var {
				data
			} = await fetch(`https://api.lolhuman.xyz/api/cnnindonesia/internasional?apikey=maslent`)
			reply(lang.wait)
		...
		case 'infogempa': {
			var {
				data
			} = await fetch(`https://api.lolhuman.xyz/api/infogempa?apikey=maslent`)
			reply(lang.wait)
		...
		case 'lirik': {
			...
			var {
				data
			} = await axios.get(`https://api.lolhuman.xyz/api/lirik?apikey=maslent&query=${text}`)
			reply(data.result)
		}
		break
		case 'infocuaca':
		case 'cuaca': {
			...
			var {
				data
			} = await axios.get(`https://api.lolhuman.xyz/api/cuaca/${args[0]}?apikey=maslent`)
		...
		case 'kodepos': {
			...
			var {
				data
			} = await axios.get(`https://api.lolhuman.xyz/api/kodepos?apikey=maslent&query=${text}`)
		...
		case 'jadwalbola': {
			var {
				data
			} = await axios.get(`https://api.lolhuman.xyz/api/jadwalbola?apikey=maslent`)
			reply(lang.wait)
		...
		case 'pasangan': {
			...
			if (anu.status == false) return reply(anu.message)
			alpha.replyWithPhoto({
				url: anu.message.gambar
			}, {
				caption: ` *Nama Anda :* ${anu.message.nama_anda}\n *Nama Pasangan :* ${anu.message.nama_pasangan}\n *Sisi Positif :* ${anu.message.sisi_positif}\n *Sisi Negatif :* ${anu.message.sisi_negatif}`
			})
		}
		break
		...
		case 'tarot': {
			...
			if (anu.status == false) return reply(anu.message)
			alpha.replyWithPhoto({
				url: anu.message.image
			}, {
				caption: ` *Lahir :* ${anu.message.tgl_lahir}\n *Simbol Tarot :* ${anu.message.simbol_tarot}\n *Arti :* ${anu.message.arti}\n *Catatan :* ${anu.message.catatan}`
			})
		}
		break
		case 'fengshui': {
